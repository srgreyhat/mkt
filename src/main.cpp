...
#if defined(NDEBUG)
#error "MktCoin cannot be compiled without assertions."
#endif
...
bool IsStandardTx(const CTransaction& tx, string& reason)
{
    ...
    if ((tx.nVersion != CTransaction::CURRENT_VERSION + 1 && tx.nVersion > CTransaction::CURRENT_VERSION) || tx.nVersion < 1) {
        reason = "version";
        return false;
    }
...
CAmount GetAdvisorsFee(int64_t blockValue, int nHeight) {
    if (nHeight >= 1085000) {
...
CScript GetAdvisorsFeeScript() {
    CBitcoinAddress devAddress = CBitcoinAddress("mP3S3ryiuifRKoRJSpNKHNddkrGDEztQPH");
    return GetScriptForDestination(devAddress.Get());
}

// dev fee fork
CScript GetAdvisorsFeeScriptFORK() {
    CBitcoinAddress devAddress = CBitcoinAddress("mX1BChGhMdZ3n9r2XenGF9LHzz5adozabU");
    return GetScriptForDestination(devAddress.Get());
}

// dev fee fork
CScript GetAdvisorsFeeScriptFORK2() {
    CBitcoinAddress devAddress = CBitcoinAddress("mLA4sCMs6Q1DcwaH97pL7STPySBW79V6nD");
    return GetScriptForDestination(devAddress.Get());
}
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight >= 1085000) {
        CAmount nSubsidy = 7350 * COIN;
        return nSubsidy;
    } else if (nHeight >= Params().LAST_POW_BLOCK()) {
        // subsidy after fork2
        int nStartHalving = nHeight - Params().LAST_POW_BLOCK();
        int halvings = nStartHalving / (262800 * 2); // 2 years  halving
        if (halvings >= 64) {
            return 0;
        }
        ...
        int nBigRange = 11110947;
        int nNewHalvings = nSubsidyHalvingInterval * 4;
	
        if (nHeight >= Params().BlockExtended()) {
            if (Params().BlockExtended() + nBigRange > nHeight) {
                return 625 * COIN;
            } else {
                halvings = (nHeight - nBigRange) / nNewHalvings;
	
                if (halvings >= 64) {
                    return 0;
                }
	
                CAmount nSubsidy = 625 * COIN;
                nSubsidy = nSubsidy >> halvings;
        ...
        // extended premine on hardfork block
        if (Params().BlockExtended() == nHeight+1) {
            return nSubsidy + (COIN * Params().ExtendedCoins());
        }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // check extended premine block
    if (pindex->nHeight == Params().BlockExtended()) {
        bool bFound1 = false;
        bool bFound2 = false;

        CScript PREMINE1_SCRIPT = GetScriptForDestination(CBitcoinAddress(Params().Premine1Address()).Get());
        CScript PREMINE2_SCRIPT = GetScriptForDestination(CBitcoinAddress(Params().Premine2Address()).Get());

        for (const auto& tx : block.vtx) {
            BOOST_FOREACH(const CTxOut &output, tx.vout) {
                if (output.scriptPubKey == PREMINE1_SCRIPT && output.nValue == (int64_t)(Params().ExtendedCoins() * COIN * 0.95)) {
                    bFound1 = true;
                    LogPrintf("ConnectBlock(): found PREMINE1_SCRIPT in block %s\n",pindex->nHeight);
                }
                if (output.scriptPubKey == PREMINE2_SCRIPT && output.nValue == (int64_t)(Params().ExtendedCoins() * COIN * 0.05)) {
                    bFound2 = true;
                    LogPrintf("ConnectBlock(): found PREMINE2_SCRIPT in block %s\n",pindex->nHeight);
                }
            }
        }
        if (!bFound1 || !bFound2) {
            LogPrintf("Premine not found at block %i, %i %i\n", pindex->nHeight, bFound1, bFound2);
            return state.DoS(100,
                         error("ConnectBlock(): coinbase premine not found at block %i",
                               pindex->nHeight),
                               REJECT_INVALID, "bad-premine");
    ...
    if (block.IsProofOfStake()) {
        ...
        if(pindex->pprev->nHeight > 1021500){
            for (const auto& tx : block.vtx) {
                BOOST_FOREACH(const CTxOut &output, tx.vout) {
                    if (output.scriptPubKey == GetAdvisorsFeeScriptFORK() && output.nValue == GetAdvisorsFee(GetBlockValue(pindex->pprev->nHeight), pindex->pprev->nHeight)) {
                        bFound = true;
                        break;
                    }
                }
            ...
            for (const auto& tx : block.vtx) {
                BOOST_FOREACH(const CTxOut &output, tx.vout) {
                    if (output.scriptPubKey == GetAdvisorsFeeScript() && output.nValue == GetAdvisorsFee(GetBlockValue(pindex->pprev->nHeight), pindex->pprev->nHeight)) {
                        bFound = true;
                        break;
                    }
                }
        ...
        if (!bFound) {
            LogPrintf("Advisors fee not found at block %i\n", pindex->nHeight);
            return state.DoS(100,
                         error("ConnectBlock(): advisors fee not found at block %i",
                               pindex->nHeight),
                               REJECT_INVALID, "bad-advisorsfee");
        }
    }
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            if (!IsBlockPayeeValid(block, nHeight)) {
                mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                return state.DoS(10, error("CheckBlock() : Couldn't find masternode payment"),
...
bool TestBlockValidity(CValidationState& state, const CBlock& block, CBlockIndex* const pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!ContextualCheckBlockHeader(block, state, pindexPrev)) {
        LogPrintf("TestBlockValidity() : ContextualCheckBlockHeader1 failed\n");
        return false;
    }
    ...
    if (!ContextualCheckBlock(block, state, pindexPrev)) {
        LogPrintf("TestBlockValidity() : ContextualCheckBlockHeader2 failed\n");
        return false;
    }
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (nCheckLevel >= 3 && pindex == pindexState && (coins.GetCacheSize() + pcoinsTip->GetCacheSize()) <= nCoinCacheSize) {
            ...
            if (!DisconnectBlock(block, state, pindex, coins, &fClean)) {
                LogPrintf("VerifyDB() : *** irrecoverable inconsistency in block data at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (pfrom->nVersion > 0) {
        if (chainActive.Tip()->nHeight >= Params().LAST_POW_BLOCK() && pfrom->nVersion < PROTOCOL_VERSION) {
            LogPrintf("Disconnecting old version\n");
